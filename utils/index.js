...
exports.rmFile = function rmFile(
  cwd,
  executable = "rm",
  color
) {
  console.log(`\n\n# ${color("rm package-lock.json and node_modules")}`);
  console.log("# ========================\n");
  return runCommand(executable, ["-rf", "package-lock.json", "node_modules"], {
    cwd,
  });
};
...
exports.gitInit = function gitInit(
  ...
  console.log(`\n\n# ${color("Git init project")}`);
  console.log("# ========================\n");
  return runCommand(executable, ["init"], {
    cwd,
  }).then(() => {
    if (remote) {
      console.log(`\n\n# ${color("git remote add origin")}`);
      console.log("# ========================\n");
      return runCommand(executable, ["remote", "add", "origin", remote], {
        cwd,
      });
    }
  });
